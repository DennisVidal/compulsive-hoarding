...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
		...
		public AudioClip[] dirtySteps;
		public AudioClip[] cobbleSteps;
		public float initialDirt_NormalRatio = 0.7f;
		float dirt_normalRatio;
		...
		void Start()
        {
			...
			dirt_normalRatio = initialDirt_NormalRatio;
			//Custom Logic ---------
		...
		private void InitiateTeleportFade()
		{
			...
			//Custom Code: -------------------------------
			PlayStepsClips(headAudioSource);
		...
		//Custom Logic
		private void PlayStepsClips (AudioSource source)
        {
			CalculateDirt_NormalRatio();
			//headAudioSource.volume = 2;
			for(int i = 0; i < 2; i++)
            {
				if(source.transform.position.x > -8.324f) // Vorerst Hardgecodete Türschwelle!
				{
					if (Random.Range(0.0f, 1.0f) < dirt_normalRatio)
						source.clip = dirtySteps[Random.Range(0, dirtySteps.Length)];
					else
						source.clip = normalSteps[Random.Range(0, normalSteps.Length)];
				}
				else
				{
					source.clip = cobbleSteps[Random.Range(0, cobbleSteps.Length)];
				}
				source.Play();
			}
		}

		private void CalculateDirt_NormalRatio()
        {
			dirt_normalRatio = initialDirt_NormalRatio * valueHolderStemvr.trashvalue;
        }
	}
}
